...
for ss = 1:length(PS_dB)
    for rr = 1:length(rho)
        ...
        % count outage events
        count = snrFe2e < g2;
        OP_F_sim(rr) = sum(count)/SimTimes;
        %% Analysis
        ...
        %
        if g2 < theta
            fun_temp = @(x) exp(-x./lSN - g2./lNF./c./x);
            OP_F_exact(rr) =  1 - exp(-mu_a/lSN)...
                + (1 - exp(-mu_b/lSF)) ...
                * (exp(-mu_a/lSN) - 1/lSN*integral(fun_temp,mu_a,inf));
            %
            OP_F_approx(rr) = 1 - exp(-mu_a/lSN - mu_b/lSF)...
                - (1 - exp(-mu_b/lSF)) ...
